...
func enter() -> void:
	...
	Audio.play_spacial_sound( DASH_AUDIO, player.global_position )
...
func exit() -> void:
	...
	Audio.play_spacial_sound( BOOM_AUDIO, player.global_position )
	...
	if ground_slam_shape_cast.is_colliding():
		for i in ground_slam_shape_cast.get_collision_count():
			...
			if c.get_parent() is Breakable:
				...
				b.queue_free()
				Audio.play_spacial_sound( b.destroy_audio, pos )
				...
				Audio.play_spacial_sound( BREAK_WOOD_AUDIO, pos )
		return true
	return false
